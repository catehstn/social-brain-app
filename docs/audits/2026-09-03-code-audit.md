...
6. **LinkedIn CSV CTR is off by 100× for CTR ≤ 1%** (`LinkedInImporter.swift:98-103`). Column is literally `CTR (%)`; heuristic `v > 1 ? v/100 : v` maps `0.72%` to 72%. Test only uses values > 1. Same shape in `SubstackImporter.swift:99-105`. Confirmed by probe.
   ...
   > **Correction (2026-09-06, #107).** "Column is literally `CTR (%)`" is **wrong**, and "confirmed by probe" applies only to the heuristic's arithmetic, not to the header. No real LinkedIn export has ever been observed with that header — the string came from the importer's own doc comment, written in the same commit as the importer (451be0f), and this audit repeated it as fact. Real Page exports head the column `Click through rate (CTR)` and store a **fraction**, so magnitude was already correct there; forcing a percentage reading would have created a fresh 100× error the other way. The Substack half stands: a literal `"0.5%"` did parse as `0.5`. #70 remains open pending a real export.
...
26. **Buffer sends the token as `?access_token=`** on every request (`BufferCollector.swift:143-152`); `httpError` echoes 200 bytes of response body into the Run screen. Confirmed.
    ...
    > **Correction (2026-09-21, #117).** The Buffer half is now fixed too, by leaving v1 rather than patching it: the collector moved to Buffer's GraphQL API, which takes the key as an `Authorization: Bearer` header — verified against a live key. Closes #124. The same change settles item 11's Buffer entry.
